...
# Test case C: psi convergence --> fixed n_NICE = 1000 
...
psi_list_n_TORAX_25 = [
    eq.time_slice[0].profiles_1d.psi for eq in equilibria_n_TORAX_25
]
...
psi_list_n_TORAX_100 = [
    eq.time_slice[0].profiles_1d.psi for eq in equilibria_n_TORAX_100
]
...
rmse_psi_list_n_TORAX_25 = [
    compute_rmse(psi_list_n_TORAX_25[i-1], psi_list_n_TORAX_25[i],
                 rho_tor_norm_list_n_TORAX_25[i-1], rho_tor_norm_list_n_TORAX_25[i])
    for i in range(1, len(psi_list_n_TORAX_25))
]
...
rmse_psi_list_n_TORAX_100 = [
    compute_rmse(psi_list_n_TORAX_100[i-1], psi_list_n_TORAX_100[i],
                 rho_tor_norm_list_n_TORAX_100[i-1], rho_tor_norm_list_n_TORAX_100[i])
    for i in range(1, len(psi_list_n_TORAX_100))
]
...
rmse_data = {
    ...
    25: rmse_psi_list_n_TORAX_25,
    30: rmse_psi_list_n_TORAX_30,
    50: rmse_psi_list_n_TORAX_50,
    100: rmse_psi_list_n_TORAX_100,
}
...
#plt.title("Case C: RMSE of $\psi$ profiles between successive iterations\nfor varying $n_{TORAX}$ (fixed $n_{NICE}=1000$)")
plt.yscale('log')
plt.xlabel("Iteration", fontsize=18)
plt.xticks(range(1, max_len + 1), fontsize=18)  # consistent ticks across cases
plt.ylabel("RMSE of $\psi$ (Wb)", fontsize=18)
