...
# Test case B: psi convergence --> fixed n_TORAX = 25
...
psi_list_n_NICE_25 = [
    eq.time_slice[0].profiles_1d.psi for eq in equilibria_n_NICE_25
]
...
psi_list_n_NICE_100 = [
    eq.time_slice[0].profiles_1d.psi for eq in equilibria_n_NICE_100
]
...
psi_list_n_NICE_1000 = [
    eq.time_slice[0].profiles_1d.psi for eq in equilibria_n_NICE_1000
]
...
rmse_psi_list_n_NICE_25 = [
    compute_rmse(psi_list_n_NICE_25[i-1], psi_list_n_NICE_25[i],
                 rho_tor_norm_list_n_NICE_25[i-1], rho_tor_norm_list_n_NICE_25[i])
    for i in range(1, len(psi_list_n_NICE_25))
]


rmse_psi_list_n_NICE_100 = [
    compute_rmse(psi_list_n_NICE_100[i-1], psi_list_n_NICE_100[i],
                 rho_tor_norm_list_n_NICE_100[i-1], rho_tor_norm_list_n_NICE_100[i])
    for i in range(1, len(psi_list_n_NICE_100))
]

rmse_psi_list_n_NICE_1000 = [
    compute_rmse(psi_list_n_NICE_1000[i-1], psi_list_n_NICE_1000[i],
                 rho_tor_norm_list_n_NICE_1000[i-1], rho_tor_norm_list_n_NICE_1000[i])
    for i in range(1, len(psi_list_n_NICE_1000))
]
...
rmse_data = {
    25: rmse_psi_list_n_NICE_25,
    100: rmse_psi_list_n_NICE_100,
    1000: rmse_psi_list_n_NICE_1000,
}
...
plt.title("Case B: RMSE of $\psi$ profiles between successive iterations\nfor varying $n_{NICE}$ (fixed $n_{TORAX}=25$)")
plt.yscale('log')
